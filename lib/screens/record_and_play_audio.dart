...
class _RecordAndPlayScreenState extends State<RecordAndPlayScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Color(0xFF494E64), // Set background color to black
        title: Text('Baby Cry Classification',
            style: TextStyle(
              fontFamily: 'Arial',
              fontSize: 20,
      ...
      body: Stack(
        children: [
          if (_recordProvider.predictionResult != null)
            ...
            _buildContent(_recordProvider,
                _playProvider), // Existing content when no prediction
          Positioned(
            bottom: 0,
            left: 0,
            right: 0,
          ...
          Positioned(
            bottom: 80, // Adjust bottom value as needed
            right: 16, // Adjust right value as needed
            child: ChatMessageButton(
              // Positioning ChatMessageButton widget
              onPressed: () {
                Navigator.push(
                  context,
  ...
  Widget _commonIconSection() {
    ...
    return Column(
      children: [
        ...
        SizedBox(height: 10),
        SizedBox( // Fix the height of the text widget
          height: 30, // Set a fixed height for the text widget
          child: Text(
            _recordProvider.isRecording ? 'Wait, please' : 'Tap to Listen',
            style: TextStyle(
              color: Colors.white,
              fontSize: 18,
  ...
  Widget _recordHeading() {
    return Container(); // Return an empty container instead of the text widget
  }
  ...
  Widget _recordingSection() {
      ...
      return InkWell(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 20), // Add space below the record button
          ],
        ),
      );
    }
    ...
    return InkWell(
      ...
      child: Column(
        children: [
          _commonIconSection(),
          const SizedBox(height: 200), // Add space below the record button
        ],
      ),
    );
  }
...
class ChatMessageButton extends StatefulWidget {
  ...
  const ChatMessageButton({Key? key, required this.onPressed})
      : super(key: key);
...
class _ChatMessageButtonState extends State<ChatMessageButton>
  ...
  Widget build(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(top: 40), // Adjust the top padding as needed
